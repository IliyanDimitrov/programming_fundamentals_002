...
describe("catalogueService", () => {
  ...
  describe("catalogueService.countBooksByKeyword", () => {
    ...
    test("returns the number of books containing the given number", () => {
      expect(catalogueService.countBooksByKeyword(2666)).toBe("Invalid Input");
    });

    test("returns the number of books containing the given Boolean", () => {
      expect(catalogueService.countBooksByKeyword(true)).toBe("Invalid Input");
    });

    test("returns the number of books containing the given array", () => {
      expect(catalogueService.countBooksByKeyword([1, 2, 3])).toBe("Invalid Input");
    });
  });
